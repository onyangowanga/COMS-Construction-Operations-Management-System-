...
export function ProcurementForm({
  ...
  useEffect(() => {
    const loadNextReference = async () => {
      // Keep the existing reference during edit mode.
      if (initialValues?.id) {
        return;
      }
      ...
      try {
        const preview = await procurementService.getNextReference(selectedProjectId);

        if (!canEditReference) {
          setValue('reference_number', preview.lpo_number);
          return;
        }
  ...
  }, [selectedProjectId, canEditReference, initialValues?.id, setValue, getValues]);

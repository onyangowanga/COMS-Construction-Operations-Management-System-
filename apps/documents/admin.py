...
class DocumentVersionInline(admin.TabularInline):
    ...
    extra = 0
    fields = ['version_number', 'file_path', 'is_latest', 'uploaded_by', 'created_at', 'notes']
...
class DocumentVersionAdmin(admin.ModelAdmin):
    list_display = ['document', 'version_number', 'is_latest', 'uploaded_by', 'created_at']
    list_filter = ['is_latest', 'created_at']
    search_fields = ['document__name', 'notes', 'file_path']
    ...
    fieldsets = (
        ...
        (_('File'), {
            'fields': ('file_path', 'uploaded_by', 'notes')
        }),
        (_('Timestamps'), {
            'fields': ('created_at',),
            'classes': ('collapse',)
